...
public class GiveawayManager {
    ...
    public Giveaway addEntry(Giveaway giveaway, String id, String nick) {
        ...
        putGiveaway(giveaway);
        instance.getDBUtil().saveEntries(giveaway).exceptionally(ex -> {
            instance.getLogger().severe("Failed to save entries for giveaway: " + giveaway.name() + ". Error: " + ex.getMessage());
            return null;
        }); // Save entries to the database immediately after adding an entry
        return giveaway;
    }
    ...
    public Giveaway giveawayFromConfig(String name) {
        ...
        instance.getDBUtil().refreshEntries(giveaway).thenAccept(entries -> {
            giveaway = giveaway.withEntries(entries);
            giveaway = giveaway.withEndTime(giveaway.endTime());
            giveaway = giveaway.withStartTime(giveaway.startTime());
            putGiveaway(giveaway);
        }).exceptionally(ex -> {
            instance.getLogger().severe("Failed to refresh entries for giveaway: " + giveaway.name() + ". Error: " + ex.getMessage());
            return null;
        });
        return giveaway;
    }
